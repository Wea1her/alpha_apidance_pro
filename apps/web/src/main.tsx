...
type Project = {
  ...
  status: ProjectStatus; reportStatus: string | null; reportVersion?: number | null;
...
function DetailPanel({ project, onClose, onRefresh }: { project: Project; onClose: () => void; onRefresh: () => void }) {
  const [signals, setSignals] = useState<Signal[]>([]); const [tweets, setTweets] = useState<Signal[]>([]); const [note, setNote] = useState(''); const [saving, setSaving] = useState(false); const [message, setMessage] = useState(''); const [versions, setVersions] = useState<ReportVersion[]>([]); const [report, setReport] = useState<ReportDocument | null>(null); const [reportVersion, setReportVersion] = useState('');
  ...
  useEffect(() => { void api<{ items: ReportVersion[] }>(`/api/projects/${project.id}/reports`).then(async (payload) => { setVersions(payload.items); const latest = payload.items[0]; if (latest) { setReportVersion(String(latest.version)); const detail = await api<{ item: ReportDocument }>(`/api/projects/${project.id}/reports/${latest.version}`); setReport(detail.item); } }).catch(() => { setVersions([]); setReport(null); }); }, [project.id]);
  ...
  const loadReport = async (version: string) => { setReportVersion(version); try { const detail = await api<{ item: ReportDocument }>(`/api/projects/${project.id}/reports/${version}`); setReport(detail.item); } catch { setReport(null); } };
...
function ReportsView({ projects, onOpen }: { projects: Project[]; onOpen: (project: Project) => void }) {
  ...
  return <section className="workspace-panel"><div className="panel-heading"><div><p className="eyebrow">AI RESEARCH</p><h2>AI 调研报告</h2><p>报告以可读文档展示，版本只增不覆盖。</p></div><span className="panel-count">{reports.filter((item) => item.versions.length).length} 个项目</span></div>{reports.length ? <div className="report-grid">{reports.map(({ project, versions }) => <button className="report-card" key={project.id} onClick={() => onOpen(project)}><div className="report-card-top"><span className="project-icon small">{project.avatarUrl ? <img src={project.avatarUrl} alt="" /> : project.name.slice(0, 1)}</span><div><strong>{project.name}</strong><small>{project.handle}</small></div><span className={`status-pill ${project.reportStatus === 'ready' ? 'ready' : ''}`}>{project.reportStatus ? (REPORT_LABEL[project.reportStatus] ?? project.reportStatus) : '待生成'}</span></div><div className="report-card-bottom"><span>{versions.length ? `最新 V${versions[0].version}` : '暂无版本'}</span><span>{formatDateTime(project.latestSignalAt)} ↗</span></div></button>)}</div> : <div className="empty panel-empty"><strong>还没有可阅读的报告</strong><span>初筛通过后，系统会自动收集证据并生成中文报告。</span></div>}</section>;
}
