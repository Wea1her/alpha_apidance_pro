...
- 风险必须具体：Rug 历史必须优先使用“Rug 证据状态”和“Rug 结论”；不能只写“暂无直接证据”，还要结合类似项目结局、删帖频率、评论区负面反馈判断风险等级；如果存在“合约相关删帖原文”，必须在第 7 节明确写“发现 CA/合约相关删帖”，在本节最后单独引用这些原文并说明风险含义，但不能只凭这一点直接判定跑路。
...
7. Rug 历史/风险：先写明 Rug 证据状态属于“未查询或查询失败 / 查询成功但无直接证据 / 有负面噪声但相关性不足 / 有明确风险证据”哪一类，再基于删帖记录、删帖频率、负面提及、评论区负面样本、类似项目结局判断是否存在跑路、骗局、严重负面历史；没有直接证据时只能写“未发现直接 Rug 证据”，不能写成“确认无 Rug 历史”；如果有合约相关删帖原文，必须明确写“发现 CA/合约相关删帖”，并把原文放在本节最后；不能只凭这一点直接判定跑路。
