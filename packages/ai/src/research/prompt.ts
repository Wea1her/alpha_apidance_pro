...
export function buildResearchReportPrompt(input: ResearchPromptInput): { system: string; user: string } {
  const user = [
    ...
    '3. 优点：以 1-3 段完整连贯的中文分析输出，不要写成关键词列表；每个数组元素至少 80 个中文字符，必须同时引用至少两类事实：一类是项目实际产品/机制/生态（例如 NFT、社交网络、Launchpad、链或玩法），另一类是账号行为或交付/用户事实（最近帖子内容、帖子热度、发帖频率、粉丝规模、用户反馈、上线/测试进展等）。然后解释这些事实为何构成优势、对短期投机/创业机会有什么意义以及证据边界。禁止只改写简介，禁止把“账号资料：”“当前信号事实：”、共同关注、Alpha 时间、evidence ID 或触发账号指标原样塞进优点。',
    '4. 缺点：以 1-3 段完整连贯的中文分析输出，不要写成关键词列表；每个数组元素至少 80 个中文字符，必须结合该项目的具体产品/赛道与至少一项账号行为或交付事实，指出团队/审计/产品/数据/竞争方面的具体缺口、对落地/留存/流动性的影响和下一步验证动作。不能套用与项目无关的泛化风险，不能只复述简介；禁止出现“账号资料：”“当前信号事实：”、共同关注、Alpha 时间、evidence ID 或触发账号的粉丝数。',
