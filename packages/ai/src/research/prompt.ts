...
export function buildResearchReportPrompt(input: ResearchPromptInput): { system: string; user: string } {
  ...
  const user = [
    ...
    '3. 优点：输出 3-5 条，每条至少包含“具体事实 → 为什么构成优势 → 对短期投机/创业机会的意义”，避免空泛形容词。',
    '4. 缺点：输出 3-5 条，每条至少包含“缺失或负面事实 → 对项目判断的影响 → 需要如何验证”，覆盖产品、执行、流动性、竞争和数据限制。',
    '5. 关注理由：用 1-3 个段落综合当前进展、优缺点和信号强度，明确是否值得跟踪、适合何种小仓试错、升级观察条件、停止跟踪条件及主要不确定性；不要把“新股申购/传统股票研究”当作本项目的判断标准。',
