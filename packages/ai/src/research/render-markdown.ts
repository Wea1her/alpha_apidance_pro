...
function paragraphs(items: readonly string[]): string { return items.length ? items.join('\n\n') : '暂无'; }
...
export function renderReportMarkdown(report: ReportDocument, generatedAt = new Date()): string {
  return [
    ...
    '## 三、优点',
    '',
    paragraphs(report.focusReason.strengths),
    '',
    '## 四、缺点',
    '',
    paragraphs(report.focusReason.weaknesses),
    '',
    '## 五、关注理由',
    '',
