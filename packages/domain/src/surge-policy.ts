...
export function evaluateSurge(observations: readonly CommonFollowObservation[], now: Date, options: SurgePolicyOptions = {}): SurgeDecision {
  const cutoff = now.getTime() - (options.windowMs ?? 60 * 60 * 1000);
  ...
  if (peakCount - baselineCount < (options.increaseThreshold ?? 5)) return { triggered: false, baselineCount, peakCount };
